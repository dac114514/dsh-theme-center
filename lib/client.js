window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// src/client/compat-settings-scope.ts
		...
		var BridgeScopeController = class {
		  ...
		  accept(view, publish, writable) {
		    if (!publish) return;
		    const decoded = this.spec.decode ? this.spec.decode(view.value) : view.value;
		    this.store.update((draft) => {
		      draft.status = "ready";
		      draft.value = decoded;
		...
		function snapshotIsReadyAndWritable(snapshot) {
		  return snapshot.status === "ready" && snapshot.writable !== false;
		}
		function bindCompatSettingsScope(ctx, spec) {
		  const official = ctx.settingsScope.bind(spec);
		  const bridge = new BridgeScopeController(createBridgeApi(fetch), spec);
		  let active = official;
		  const chooseActive = () => {
		...
		function createStore() {
		  return (0, import_client2.defineStore)({
		    init: () => ({
		      ready: false,
		      active: "system",
		      custom: [],
